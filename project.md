...
├── src/
│   ├── assets/                 # 이미지 및 미디어 자산
│   │   ├── hero.png            # 팀 매칭용 히어로 이미지 (스타크래프트 컨셉)
│   │   ├── react.svg
│   │   └── vite.svg
│   ├── pages/                  # 개별 메뉴 페이지 컴포넌트
│   │   ├── Home.tsx            # 메인 홈 화면 (각 기능으로의 내비게이션 카드 제공)
│   │   ├── Ranking.tsx         # Billizone 연동 9샷 멤버 랭킹 대시보드
│   │   ├── Settlement.tsx      # 모임비/당구비 차수별 N분의 1 정산 계산기
│   │   └── Random.tsx          # 스타크래프트 균형 잡힌 팀 매칭기
...
- **그린 펠트 당구대 테마 (기본 & 랭킹)**: 당구대 천 느낌의 소프트 펠트 그린 배경 및 화이트톤 글래스모피즘 카드 UI
- **클래식 당구 펠트 그린 액센트**: `#0f766e` (Simonis Green) 기반의 소프트 글로우 효과
- **스타크래프트 프로토스 테마 (팀 매칭)**: 테두리 네온 글로우 효과, 공상과학 느낌의 어두운 다크블루 톤과 골드 액센트 컬러 구성
- **마이크로 애니메이션**: `fadeIn`, `slideUp` 트랜지션 적용으로 매끄러운 화면 흐름 제공
- **반응형 레이아웃**: CSS Grid 및 미디어 쿼리를 사용하여 모바일 모드(768px 이하)에서도 편리하게 사용 가능
...
- **실시간 요약**: 전체 금액 합계 및 차수별 참석자가 표시되며, 오른쪽 패널에 멤버별로 내야 할 최종 금액을 실시간 계산하여 정렬.
- **카카오톡 공유 포맷 뷰**: 복사하여 바로 공유하기 편리한 텍스트 리스트 형태의 정산 요약 자동 렌더링.
...
- **최대 9명 지원**: 홀수 인원(예: 5명, 7명) 참여 시 컴퓨터(실력 점수: 10)를 자동으로 배치하여 짝수가 되도록 매칭 보완.
- **승률 예측**: 각 팀의 총점 비율을 기반으로 예상 승률(%)을 시각적으로 표시.
